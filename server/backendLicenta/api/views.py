...
def add_log(log_data, err=False):
    ...
    path = os.path.expanduser("~") + "/RemoteMonitor_admin/"
    if err:
...
@api_view(['POST'])
def logout(request):
    add_log(request.POST["username"] + " logged out", err=False)
...
def add_device(request):
    ...
    command1 = "gnome-terminal -- bash -c './backendLicenta/initialize.sh " + user + " " + ip + "'"
    print(command1)
    ...
    add_log("deivce: " + user + "@" + ip + " has been connected by " + username, err=False)
    response = "OK"
...
def reconnect(request):
    ...
    client = Client.objects.get(pk=ip)
    client.isReconnecting = "True"
    client.save()

    command = "ssh " + user+"@"+ip + " \" nohup python3 ~/RemoteMonitor/TCP_receive.py\""
    subprocess.Popen(command, shell=True)
    add_log("deivce: " + user + "@" + ip + " reconnected by " + username, err=False)
    response = "success"
...
def delete_client(request):
    ...
    if client.delete() and received != "none":
        add_log("device: " + client.user + "@" + clientIP + " has been deleted by " + username, err=False)
        response = "success"
    else:
...
def remove_client(request):
    ...
    if client.delete():
        add_log("device: " + client.user + "@" + clientIP + "has been removed by " + username, err=False)
        response = "success"
    else:
...
def shutdown_client(request):
    ...
    try:
        received = perform_shutdown(client)
        add_log("device " + client.user + "@" + clientIP + " has been shut down by " + username, err=False)
    except:
...
def run_command(request):
    ...
    try:
        ...
        add_log("admin " + username + "ran '" + command + "' on device " + client.user + "@" + clientIP, err=False)
        resultList = str(result, "utf-8").split('\n')
        print(resultList)
...
def upload_file(request):
    ...
    if request.method == "POST":
        ...
        if script:
            ...
            responseData.append({"result": "no_file"})

        add_log("file was uploaded to device " + client.user + "@" + clientIP + " by " + username, err=False)
        ...
        if time != "run":
            command = "scp /tmp1/remoteScript " + client.user + "@" + client.ip + ":~/RemoteMonitor/" + fileName + " 2&> ~/RemoteMonitor/errorOutput.txt"
            with open("~/RemoteMonitor/errorOutput.txt", "r") as f:
                responseData.append({"error": f.readline()})
            subprocess.check_output(command, shell=True)
...
def set_permissions(request):
    ...
    if User.objects.get(username=activeUser).is_staff:
        ...
        return Response("permissions_error")
    return Response("OK")
